...
export function NewContractModal({ isOpen, onClose, onSuccess, initialBuyerId, initialContact }: NewContractModalProps) {
  ...
  const selectedContact = useMemo(() => {
    const fromList = contacts.find(c => c.id === formData.buyerId);
    if (fromList) return fromList;
    if (initialContact && initialContact.id === formData.buyerId) return initialContact;
    return null;
  }, [contacts, formData.buyerId, initialContact]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-2.5">
            <div className="space-y-0.5">
              ...
              >
                ...
                {initialContact && !contacts.some(c => c.id === initialContact.id) && (
                  <option value={initialContact.id}>{initialContact.firstName} {initialContact.lastName}</option>
                )}
                {contacts.map(c => (
